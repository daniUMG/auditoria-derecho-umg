<template>
    <v-layout>
        <v-flex>
            <MenuLeyes />
            <div class="mt-4">
                <div style="display: flex;">
                    <svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" viewBox="0 0 640 640" style="position: relative; top: 6px; left: -10px;"><path d="M118.2 126.4C101.5 120.8 92.4 102.6 98 85.9C103.6 69.2 121.7 60.1 138.5 65.6L251.5 103.3C265.4 79.8 291.1 64 320.4 64C364.6 64 400.4 99.8 400.4 144C400.4 147 400.2 149.9 399.9 152.8L522.5 193.7C539.3 199.3 548.3 217.4 542.7 234.2C537.1 251 519 260 502.2 254.4L366.7 209.2C362.2 212.4 357.4 215.1 352.3 217.4L352.3 544.1C352.3 561.8 338 576.1 320.3 576.1L128.3 576.1C110.6 576.1 96.3 561.8 96.3 544.1C96.3 526.4 110.6 512.1 128.3 512.1L288.3 512.1L288.3 217.4C267.3 208.2 251.1 190.4 244.1 168.4L118.2 126.4zM200.8 352L128.3 227.8L55.9 352L200.8 352zM128.4 448C65.5 448 13.2 414 2.4 369.1C-.2 358.1 3.4 346.8 9.1 337L104.3 173.8C109.3 165.2 118.5 160 128.4 160C138.3 160 147.5 165.3 152.5 173.8L247.7 337C253.4 346.8 257 358.1 254.4 369.1C243.6 413.9 191.3 448 128.4 448zM511.2 355.8L438.8 480L583.7 480L511.3 355.8zM637.2 497.1C626.4 542 574.1 576 511.2 576C448.3 576 396 542 385.2 497.1C382.6 486.1 386.2 474.8 391.9 465L487.1 301.8C492.1 293.2 501.3 288 511.2 288C521.1 288 530.3 293.3 535.3 301.8L630.5 465C636.2 474.8 639.8 486.1 637.2 497.1z" :fill="`${tablero.tema}`"/></svg><h1>Leyes de <span> {{tablero.nombre}}</span></h1>
                    <v-chip color="secondary" class="ml-3" style="color: rgb(57, 162, 227) !important; background: rgb(230, 243, 251) !important; font-size: 13px; height: 22px; position: relative; top: 18px;">
                        <b>Región: </b><span style="margin-left: 5px !important;">{{ tablero.region }}</span>
                    </v-chip>
                </div>
                <div v-if="carga" class="text-center mt-5">
                    <h2>Cargando empresas...</h2>
                    <sync-loader :loading="carga"></sync-loader>
                </div>
                <v-card v-else>
                    <v-card-title>
                        <v-text-field
                            v-model="search"
                            append-icon="fas fa-search"
                            label="Search"
                            single-line
                            hide-details
                        ></v-text-field>
                    </v-card-title>
                    <v-data-table
                        :headers="headers"
                        :items="leyes"
                        :search="search"
                    >
                        <template v-slot:[`item.actions`]="{item}">
                            <v-list-item :to="`/legislacion/editar/ley/${item.id}`">
                                <v-icon class="">fas fa-eye</v-icon>
                            </v-list-item>
                            <v-list-item :to="`/auditar/ley/${item.id}`" v-if="usuario.tipo === 'auditor'">
                                <v-icon left color="">mdi-clipboard-edit-outline</v-icon>
                            </v-list-item>
                        </template>
                    </v-data-table>
                </v-card>
            </div>
        </v-flex>
    </v-layout>
</template>

<script>
import { mapActions, mapState } from "vuex"
import MenuLeyes from '@/components/MenuLeyes'
import PulseLoader from 'vue-spinner/src/PulseLoader.vue'
import SyncLoader from 'vue-spinner/src/SyncLoader.vue'

export default {
    name: 'Leyes',
    data () {
      return {
        search: '',
        headers: [
            { text: 'Decreto', value: 'decreto' },
            { text: 'Fecha Publicación', value: 'fpublicacion' },
            { text: 'Fecha agregada a la empresa', value: 'fecha' },
            // { text: 'Auditada por', value: 'auditada_por' },
            // { text: 'Fecha Auditoría', value: 'fecha_auditoria' },
            { text: 'Acciones', value: 'actions', filterable: false}
        ]
      }
    },
    components: {
        MenuLeyes, PulseLoader, SyncLoader
    },
    computed: {
        ...mapState(['leyes','tablero','carga','usuario'])
    },
    methods: {
        ...mapActions(['getLeyesLegislacion','getEmpresa'])
    },
    created() {
        this.getLeyesLegislacion(this.$route.params.id)
        this.getEmpresa(this.$route.params.id)
    }
}
</script>