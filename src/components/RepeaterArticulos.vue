                    ...
                    <div v-for="(articulo, articuloIndex) in capitulo.articulos" :key="articulo.id" class="mb-3">
                      <v-card outlined color="green lighten-5">
                        ...
                        <v-card-text>
                          ...
                          <v-divider class="my-3"></v-divider>
                          <div class="d-flex justify-space-between align-center mb-3">
                            <h6 class="text-body-2 orange--text">Incisos del Artículo</h6>
                            <v-btn
                              color="orange"
                              x-small
                              outlined
                          ...
                          <div v-if="articulo.incisos.length === 0">
                            <v-alert type="info" outlined dense class="mb-2">
                              No hay incisos en este artículo.
                            </v-alert>
                          </div>

                          <!-- Repeater de Incisos -->
                          <div v-for="(inciso, incisoIndex) in articulo.incisos" :key="inciso.id" class="mb-2">
                            <v-card outlined color="orange lighten-5">
                              <v-card-title class="caption orange lighten-2 white--text pa-3">
                                ...
                                Inciso {{ getLetraInciso(incisoIndex) }}
                                <v-spacer></v-spacer>
                                <v-btn
                                  icon
                                  x-small
                                  color="white"
                                  @click="eliminarInciso(tituloIndex, capituloIndex, articuloIndex, incisoIndex)"
                                >
                                  <v-icon x-small>mdi-delete</v-icon>
                                </v-btn>
                              </v-card-title>
      ...
      <v-card>
        ...
        <v-card-text class="pa-6">
          <div v-if="documento.numeroLey">
            <h2 class="text-center mb-4">{{ documento.numeroLey }}</h2>
            ...
            <div v-for="titulo in titulos" :key="titulo.id" class="mb-6">
              ...
              <div v-for="capitulo in titulo.capitulos" :key="capitulo.id" class="ml-4 mb-4">
                ...
                <div v-for="articulo in capitulo.articulos" :key="articulo.id" class="ml-4 mb-3">
                  ...
                  <div v-if="articulo.incisos.length > 0" class="ml-4">
                    <div v-for="(inciso, index) in articulo.incisos" :key="inciso.id" class="mb-1">
                      <span class="font-weight-medium orange--text">{{ getLetraInciso(index) }})</span>
                      {{ inciso.contenido }}
                    </div>
                  </div>
                </div>
...
export default {
  ...
  methods: {
    ...
    agregarInciso(tituloIndex, capituloIndex, articuloIndex) {
      ...
      this.titulos[tituloIndex].capitulos[capituloIndex].articulos[articuloIndex].incisos.push(nuevoInciso)
      this.mostrarSnackbar('Inciso agregado correctamente', 'success')
    },
    
    eliminarInciso(tituloIndex, capituloIndex, articuloIndex, incisoIndex) {
      if (confirm('¿Estás seguro de eliminar este inciso?')) {
        this.titulos[tituloIndex].capitulos[capituloIndex].articulos[articuloIndex].incisos.splice(incisoIndex, 1)
        this.mostrarSnackbar('Inciso eliminado', 'warning')
      }
    },

    // MÉTODOS AUXILIARES
    getLetraInciso(index) {
      // Convierte índice a letra (a, b, c, etc.)
      ...
      return this.titulos.reduce((total, titulo) => 
        total + titulo.capitulos.reduce((subTotal, capitulo) => 
          subTotal + capitulo.articulos.reduce((incisoTotal, articulo) => 
            incisoTotal + articulo.incisos.length, 0), 0), 0)
    }
  }
}
</script>
